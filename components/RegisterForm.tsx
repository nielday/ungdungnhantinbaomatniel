...
const RegisterForm: React.FC<RegisterFormProps> = ({ onSwitchToLogin }) => {
  ...
  const validateForm = () => {
    if (!formData.phoneNumber || !formData.email || !formData.fullName || !formData.age || !formData.password || !formData.confirmPassword) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await register(formData.phoneNumber, formData.email, formData.fullName, parseInt(formData.age));
    if (success) {
      setSuccess(true);
      setTimeout(() => {
        setSuccess(false);
